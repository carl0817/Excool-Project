...
caffe_root = './caffe-model-zoo/'  # this is the path to caffe, {caffe_root}/examples
data_root = './frames/'
import caffe
...
for seg in range(1,Ns+1):
    ...
    for im in numlist:
        ...
        image = caffe.io.load_image(data_root+'/seg'+str(seg)+'/im-'+str(im+1)+'.jpg')
        transformed_image = transformer.preprocess('data', image)
        net.blobs['data'].data[...] = transformed_image
        output = net.forward()
        for lay_idx in range(0,len(layer_name_list)): 
...
for seg in range(1,Ns+1):
    ...
    for im in numlist:
        ...
        image = caffe.io.load_image(data_root+'/test'+str(seg)+'/im-'+str(im+1)+'.jpg')
        transformed_image = transformer.preprocess('data', image)
        net.blobs['data'].data[...] = transformed_image
        output = net.forward()
        for lay_idx in range(0,len(layer_name_list)): 
